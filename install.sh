...
function createDBStuff()
{
	...
	SQL5="USE ${DB_NAME};"
	SQL6="CREATE TABLE loot(id INT AUTO_INCREMENT PRIMARY KEY NOT NULL,username VARCHAR(128) NOT NULL,password VARCHAR(60) NOT NULL,service VARCHAR(20) NOT NULL,datetimestamp VARCHAR(40) NOT NULL);"
...
#branding config files
    ...
    cp $installDir/dpportal.sh /etc/dpportal/run/
    cp $installDir/HowTo/* /etc/dpportal/help/
    cp $installDir/bootstrap.sh /etc/dpportal/run/
    cp $installDir/setBrand.sh /etc/dpportal/run/
    cp $installDir/README.md /etc/dpportal/help/
    
    chmod a+x /etc/dpportal/run/dpportal.sh
    chmod a+x /etc/dpportal/run/bootstrap.sh
    chmod a+x /etc/dpportal/run/setBrand.sh
    chmod a+x /etc/dpportal/help/uninstall.sh
