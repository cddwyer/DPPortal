...
function configExists()
{
	...
	if [[ -r "$quickStartFile" ]]; then
		cfgSSIDValue=$(cat $quickStartFile | grep "^SSID=" | sed 's/SSID=//g')
		if [[ -z $cfgSSIDValue ]]; then
        	quickExists=0
			echo "Doesn't appear to have values set in the quick start config file" >> $dppErrLog
			_error "Sorry I can't read values from that file, specifically the SSID seems to be blank..."
			_die "Quitting now, I'm done with you fr!!"
		else
			quickExists=1
			echo -e "$infoA quick start file has been found.\n\nReading values..."
		fi

	else
		if [[ ! -f "$quickStartFile" ]]; then
        	quickExists=0
		else
...
function askQuestions()
{
	...
	read useChan
 	...
 	if [[ ! -z "$useChan" ]] && [[ "$useChan" != " " ]] && [[ $useChan =~ ^[0-9]+$ ]]; then
		allOptsArgs+=" -c $useChan"
	fi
    ...
    echo "Would you like to specify a BSSID for the AP? (optional, but if you do specify one, please use a colon as the delimiter)"
	read useBSSID
    
	if [[ ! -z "$useBSSID" && "$useBSSID" != " " ]]; then
		allOptsArgs+=" --mac $useBSSID"
	else
		useBSSID="6C:AA:B3:AE:FE:78"
		allOpsArgs+=" --mac $useBSSID"
	fi
	...
	if [[ "$useType" != "AC" && "$useType" != "N" ]]; then
		_error "That wasn't a valid Wi-Fi standard, the options are AC BG or N"
    	_die "Closing down..."
	fi
	...
	if [[ "$useType" == "BG" ]]; then
		$allOptsArgs+=" --ieee80211bg"
	fi
	
	writeTempConfig
	
	echo "Would you like to save these values to the auto run file? Y/N (optional)"
	read optSavetoAuto
	optSaveAns="${optSavetoAuto^^}"
	
	if [[ $optSaveAns == "Y" ]]; then
...
function writeTempConfig() {
	...
	rm -f "/etc/ddportal/config/.configUsado.cfg"
	touch "/etc/dpportal/config/.configUsado.cfg"
	tmpCfgFile="/etc/dpportal/config/.configUsado.cfg"

	#Write current set of configuration values to a temp config that can be re-used or recovered later
	echo "SSID=$useSSID" >>  $tmpCfgFile
	echo "INTERFACE=$useInt" >>  $tmpCfgFile
	echo "CHANNEL=$useChan" >> $tmpCfgFile
	echo "FREQUENCY=$useFreq" >> $tmpCfgFile
	echo "BSSID=$useBSSID" >> $tmpCfgFile
	echo "DRIVER=$useDriver" >> $tmpCfgFile
	echo "SHOWCLIENTS=$useShowClients" >> $tmpCfgFile
	echo "USETYPE=$useType" >> $tmpCfgFile
...
function startBerate()
{
	...
	#Give it a coupl of seconds to initalise
	sleep 2
    ...
    echo -e "$info\nDPPortal has started with the process ID as $thisBeratePID."
